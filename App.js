...
import {StyleSheet, ScrollView,FlatList, ImageBackground, SafeAreaView,Button, ActivityIndicator, AsyncStorage, StatusBar, TextInput,TouchableOpacity,Text, View} from 'react-native';
import { createAppContainer } from '@react-navigation/native';
import { createStackNavigator } from 'react-navigation-stack';

// Username: user / password: 12345
const userInfo={username:'user', password: '12345'};
...
//Film VIEW
  render() {
    ...
    <View style={styles.container}>
      ...
      <Text style={{fontWeight: "bold"}}>Film title: {item.title}         Category: {item.category} </Text>
      <Text>Length: {item.length}     Rating: {item.rating}</Text>
      <TouchableOpacity style={[styles.btnEnter,{ backgroundColor: '#f00', width:100, padding: 5, alignSelf:'center', marginTop: 10}]} color="red" 
      ...
      <TextInput style={styles.inputF} placeholder="Title" 
      onChangeText={(text) => this.handleChange(text, "ititle")}  />
      <TextInput style={styles.inputF} placeholder="Category"
      onChangeText={(text) => this.handleChange(text, "icategory")} />
      <TextInput style={styles.inputF} placeholder="Length"
      onChangeText={(text) => this.handleChange(text, "ilength")} 
      keyboardType={'numeric'} />
      <TextInput style={styles.inputF} placeholder="Rating"
       onChangeText={(text) => this.handleChange(text, "irating")} 
       keyboardType={'numeric'}  />
      <TouchableOpacity  style={[styles.btnEnter,{ marginBottom: 10}]} 
...
if(ititle.length < 2){
  ...
  data: [...prevState.data, {"id": iID, "title": ititle, "category": icategory, "length":ilength, "rating":irating}]
}))
 }
}
